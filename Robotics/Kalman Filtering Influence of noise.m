%%% Kalman Filtering: Influence of noise %%%
...
for i = 1:T
    ...
    y_L1_free(1,i) = C*z_L1(:,i);
    
    L1 = dlqr(A,B,Q,R); % input your control law here
    u_L1(:,i) = -L1*(zhat_L1(:,i)-zr);
    
    zhat_L1(:,i+1) = ForwardModel(A,B,C,K1,zhat_L1(:,i),y_L1_free(1,i),u_L1(1,i));
...
for i = 1:T
    ...
    y_L2_free(:,i) = C*z_L2(:,i);
    L2 = dlqr(A,B,Q,R); % input your control law here
    u_L2(:,i) = -L2*(zhat_L2(:,i)-zr);
    
    zhat_L2(:,i+1) = ForwardModel(A,B,C,K2,zhat_L2(:,i),y_L2_free(1,i),u_L2(1,i));
